...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class Unbreakable : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                float timePercent = m_Timer / m_RestoreTime;
                if (timePercent > 1.0f)
                {
                    ReplaceObject();
                }
                ...
                {
                    RestoreBrokenObject(timePercent);
                }

            }
        }
        void WaitAndSavePose()
        {
            if (m_Timer > m_RestTime)
            {
                ...
                foreach (Transform child in m_ChildTransforms)
                {
                    ...
                    ChildPoses poses = m_ChildPoses[child];
        ...
        {
            m_Restored = true;
            GameObject restoredVersion = Instantiate(m_RestoredVersion, transform.position, transform.rotation);
            m_OnRestore.Invoke(restoredVersion);
            Destroy(gameObject);
        }
        public void RestoreBrokenObject(float timePercent)
        {
            ...
            foreach (Transform child in m_ChildTransforms)
            {
                ...
                ChildPoses poses = m_ChildPoses[child];
                Vector3 lerpedPosition = Vector3.Lerp(poses.m_EndPose.position, poses.m_StartPose.position, timePercent);
                Quaternion lerpedRotation = Quaternion.Slerp(poses.m_EndPose.rotation, poses.m_StartPose.rotation, timePercent);
